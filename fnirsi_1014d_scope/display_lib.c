...
void display_draw_shaded_rect(uint32 xpos, uint32 ypos, PSHADEDRECTDATA shadeinfo, PTEXTDATA textinfo)
{
  ...
  //The shaded rectangle uses three rectangles within each other with a different shade of color
...
void display_copy_icon_full_color(const uint16 *icon, uint32 xpos, uint32 ypos, uint32 width, uint32 height)
{
  ...
  for(line=0;line<height;line++)
  {
    ...
    {
      //Copy the icon pixel to the screen
      ptr[pixel] = icon[idx++];
    }
...
void display_left_pointer(uint32 xpos, uint32 ypos, int8 id)
{
  ...
  //Draw the id
  display_character(xpos + 4, ypos, id);
}
...
void display_right_aligned_text(uint32 xpos, uint32 ypos, const char *text)
{
  ...
  //Need to calculate a new xpos based on the metrics for this font
  //For fixed width it is a case of string length times character width
  
  //Check on the type of font for the correct handling function
  if(displaydata.font->type == VARIABLE_WIDTH_FONT)
  {
    //Calculate needed x position based on the metrics for this font
...
uint32 calc_vw_string_width(const char *text)
{
  ...
  while(*text)
  {
    ...
    if(info)
    {
      ...
      //Add the width for this character to the total
      width += metrics->width;
    }
...
{
  //Get the font information for this text
  PFONTDATA           font = displaydata.font;
  ...
  while(*text)
  {
    //Sum up the total width for all the characters in the text
    width += info->width;
